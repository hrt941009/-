...
@interface ReturnMoneyViewController ()<UIActionSheetDelegate,UITextViewDelegate,UIPickerViewDataSource,UIPickerViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate>
{
    UILabel *showLabel;
    UIView *_pickerView;
    UIView *bgColorView;
...
- (void)viewDidLoad {
    ...
    _dataArray = [[NSMutableArray alloc] initWithObjects:@"退货原因",@"退货说明", nil];
    _returnResonArray = [[NSMutableArray alloc] initWithObjects:@"保质期与商品描述不符",@"收到商品少间或污渍",@"效果不好/不喜欢",@"使用后过敏",@"其他", nil];
    _returnReason = @"请选择退货原因";
...
//    self.photoView = [[MessagePhotoView alloc] initWithFrame:CGRectMake(0, 150, 320, 130)];
...
- (void)createImageView{
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 150, kScreenWidth, (kScreenWidth - 80)/3 + 20)];
...
- (void)submitApplyButtonClicked:(UIButton*)button{
    ...
    if (_textView.text.length != 0) {
        inrto = [NSString stringWithFormat:@"%@",_textView.text];
    }
    NSString *reason;
    if (_row == 0) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"请选择退货原因" delegate:nil cancelButtonTitle:MyLocalizedString(@"OK") otherButtonTitles:nil, nil];
        [alertView show];
    }else{
        reason = [NSString stringWithFormat:@"%@",_returnResonArray[_row - 1]];
        //    拼凑img的url
        NSString *imgUrl;
        ...
        [ReturnPurchaseModel getReturnPurchaseDataCode:_orderNumber
                                                 intro:inrto
                                                reason:reason
                                                 money:nil
                                                 image:imgUrl
                                                 block:^(int code, NSString *msg) {
                                                     if (code == 1) {
                                                         UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"退货申请成功！" delegate:self cancelButtonTitle:nil otherButtonTitles:MyLocalizedString(@"OK"), nil];
                                                         [alertView show];
//                                                         [self.navigationController popToRootViewControllerAnimated:YES];
                                                     }
                                                     if (code == 2) {
                                                         UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"已经提交过退货申请，请耐心等待处理。。。" delegate:nil cancelButtonTitle:MyLocalizedString(@"OK") otherButtonTitles:nil, nil];
                                                         [alertView show];
                                                     }
                                                     if (code == 0) {
...
- (void)createView{
    for (int i = 0 ; i < [_dataArray count]; i++) {
        UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(10, 15 + 55*i, kScreenWidth - 20, 40)];
...
- (void)createTextView{
    _textView = [[UITextView alloc] initWithFrame:CGRectMake(10, 100, kScreenWidth - 20, 60)];
