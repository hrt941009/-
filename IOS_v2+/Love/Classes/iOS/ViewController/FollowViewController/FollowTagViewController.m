...
- (void)getFollowTagListData:(NSNotification *)noti{
    ...
    if ([_dataArray count] == 0) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth -150, 130)];
        label.text = @"您还没有关注任何标签";
        label.font = [UIFont lovFontWitnSize:13 IsBold:NO];
        label.textAlignment = NSTextAlignmentCenter;
        [self.view addSubview:label];
        
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(8, 1, kScreenWidth - 8, 0.5)];
        lineView.backgroundColor = [UIColor colorRGBWithRed:0 green:0 blue:0 alpha:0.3];
        [sectionFive addSubview:lineView];
    }else{
